...
export const Content = () => {
  ...
  const createTask = () => {
    ...
    if(!isCreating){
      ...
      const latest_task = localTasks[localTasks.length - 1];
      ...
      localTasks.push(obj);
      setTasks(localTasks)
      // add to all tasks after save
      // trigger save saat selesai input tanggal / description / title
    }
  }
  ...
  const inputTitle = (value:string, id:number) => {
    let localAllTasks = [...allTasks]
    const taskIndex = localAllTasks.findIndex(element => element.id === id)
    localAllTasks[taskIndex].title = value
    setAllTasks(localAllTasks)
  }
  ...
  return (
    <>
        ...
        <div className='h-full p-4 pt-0'>
          ...
          //pindah ke component card
            <div key={task.title+index}>
              <div className='grid grid-cols-16 gap-2 w-full border-b-2 py-4 items-center'>
                <input className='w-full col-span-1' type={'checkbox'} checked={task.isDone} onChange={(e) => changeStatus(e.target.checked, task.id)} />
                {isCreating && task.title.length === 0 ? 
                  <input className="col-span-8 text-sm font-semibold" placeholder='Type Task Title' onChange={(e) => inputTitle(e.target.value, task.id)}/>
                :
                  <p className={`col-span-8 text-sm font-semibold ${task.isDone ? 'text-[#828282] line-through' : ''}`}>{task.title}</p>
                }
                ...
                <p className='col-span-2 text-sm'>{formatDate(task.date)}</p>
              ...
              <div className={`px-8 py-4 ${task.isCardOpen ? 'max-h-none block' : 'max-h-0 hidden'}`}>
                ...
                <div className='grid grid-cols-10'>
                  <MdOutlineModeEdit size={20} className="col-span-1 text-[#2F80ED]" />
